...
public class JwtInterceptor implements org.springframework.web.servlet.HandlerInterceptor{
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        System.out.println("经过了拦截器（全部方法都要经过）");
            ...
            if(header.startsWith("Bearer ")){  //前后端在header约定的,startsWith获取的是Bearer （固定写法）
                ...
                try {
                    ...
                    if (roles != null && roles.equals("admin")){
                        request.setAttribute("claims_admin",claims);//如果claims_admin不等于空，就是管理员
                    }
                    if (roles != null && roles.equals("user")){
                        request.setAttribute("claims_user",claims);//如果claims_user不等于空，就是管理员
                    }
                }catch (Exception e){
